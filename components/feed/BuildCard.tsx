...
export default function BuildCard({
  ...
  return (
    <div className="post">
      <Avatar avatarUrl={avatarUrl} emoji={emoji || '\u26A1'} size="small" />
      <div className="post-body">
        <div className="post-header">
          <span className="post-name">{displayName || 'Build'}</span>
          {username && <span className="post-handle">{username}</span>}
          <span className="post-time">{timeAgo(createdAt)}</span>
        </div>
        <div className="post-text">
          {repo && <>Pushed to <code>{repo}</code>{'\n'}</>}
          {commitMessage && <>{'\n'}{commitMessage}</>}
        </div>
        <div className="code-card">
          <span className="comment">// Build logged automatically</span>{'\n'}
          +{xpEarned} XP earned{streak > 0 && <> (streak: {streak} day{streak !== 1 ? 's' : ''}, {streakMultiplier.toFixed(1)}x)</>}{'\n'}
          {linesChanged > 0 && <>{linesChanged} lines changed{'\n'}</>}
        </div>
